...
# define variables --------------------------------------------------------------------------------
API_URL = "https://qugoh-aml-at3-latest.onrender.com/"
API_HEALTH_POINT = 'health/'
API_PREDICT_ND = 'predict/XRP/'
API_PREDICT_7D = 'predict/XRP/7d/'
API_DATE = 'get_date/'
...
# CSV path
csv_path = 'data/25658663-AT3-Ripple.csv'
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                pred_prices = api_7d_data['prediction']['price_high_t1']
...
def get_next_data(url_path, date = None):
    ...
    if response.status_code == 200:
        ...
        pred_prices = api_next_day['prediction']['price_high_t1']
...
def run():
    st.title("Ripple XRP Dashboard")

    st.markdown(
        """
        ## 💱 About XRP (XRP)
        **XRP**, developed by **Ripple Labs** and launched in **2012**, is a digital asset designed for **fast**, **low-cost**, and **scalable** cross-border payments. Unlike traditional cryptocurrencies that rely on mining, XRP uses the **Ripple Protocol Consensus Algorithm (RPCA)**, enabling transactions to settle in **seconds** with minimal energy consumption. Its primary goal is to facilitate seamless transfers between **financial institutions** and **payment providers**, making XRP a bridge currency for international transactions and a key player in transforming global payments infrastructure.
        """
        )
